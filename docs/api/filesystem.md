...
To use the Filesystem API, ensure that the `filesystem` module is enabled in your `neutralino.config.json` file under the `nativeAllowList` section:
...
Note: Attempting to call filesystem methods without proper permissions will result in a `NE_RT_NATPRME` error.
...
```js
let pathParts = await Neutralino.filesystem.getPathParts(
  "./myFolder/myFile.txt"
);
...
```js
await Neutralino.filesystem.setPermissions(NL_PATH + "/my-directory-1", {
  ownerRead: true,
  groupRead: true,
});
await Neutralino.filesystem.setPermissions(NL_PATH + "/my-directory-2", {
  all: true,
});
await Neutralino.filesystem.setPermissions(
  NL_PATH + "/my-directory-3",
  { otherAll: true },
  "REMOVE"
);
```
...
```js
const permissions = await Neutralino.filesystem.getPermissions(
  NL_PATH + "/my-directory-1"
);
```
...
```js
let path = await Neutralino.filesystem.getJoinedPath(
  "./myFolder",
  "audio",
  "myFile.mp3"
);
...
```js
let path = await Neutralino.filesystem.getNormalizedPath(
  ".\\myFolder\\main.js"
);
...
```js
let path = await Neutralino.filesystem.getUnnormalizedPath(
  "./myFolder/main.js"
);
console.log(path); //  `.\myFolder\main.js`
```
